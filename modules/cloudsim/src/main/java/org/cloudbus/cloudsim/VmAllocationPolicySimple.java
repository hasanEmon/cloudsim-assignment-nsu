...
public class VmAllocationPolicySimple extends VmAllocationPolicy {
	...
	public boolean allocateHostForVm(Vm vm) {
		...
		if (!getVmTable().containsKey(vm.getUid())) { // if this vm was not created
			do {// we still trying until we find a host or until we try all of them
				...
				tries++;
			} while (tries < this.getFreePes().size());
	...
	public void deallocateHostForVm(Vm vm) {
		Host host = this.getVmTable().remove(vm.getUid());
		int idx = this.getHostList().indexOf(host);
		int pes = this.getUsedPes().remove(vm.getUid());
		if (host != null) {
			host.vmDestroy(vm);
			this.getFreePes().set(idx, getFreePes().get(idx) + pes);
		}
	}

	@Override
	public Host getHost(Vm vm) {
		return this.getVmTable().get(vm.getUid());
	}

	@Override
	public Host getHost(int vmId, int userId) {
		return this.getVmTable().get(Vm.getUid(userId, vmId));
	}
